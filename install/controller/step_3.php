...
class ControllerStep3 extends Controller {
	...
	public function index() {		
		...
		if (isset($this->request->post['rtlo'])) {
			$this->data['rtlo'] = $this->request->post['rtlo'];
		} else {
			$this->data['rtlo'] = '';
		}
	...
	private function validate() {
		if (!$this->request->post['db_host']) {
			$this->error['db_host'] = 'Database host is verplicht';
		}

		if (!$this->request->post['db_user']) {
			$this->error['db_user'] = 'Database gebruikersnaam is verplicht';
		}

		if (!$this->request->post['db_name']) {
			$this->error['db_name'] = 'Database naam is verplicht';
		}
		
		if ($this->request->post['db_prefix'] && preg_match('/[^a-z0-9_]/', $this->request->post['db_prefix'])) {
			$this->error['db_prefix'] = 'Database prefix mag alleen kleine letters bevatten, cijfers en "_"!';
		}
		...
		if ($this->request->post['db_driver'] == 'mysql') {
			if (!$connection = @mysql_connect($this->request->post['db_host'], $this->request->post['db_user'], $this->request->post['db_password'])) {
				$this->error['warning'] = 'Fout: kon niet verbinden met de database, controleer server, gebruikersnaam en wachtwoord';
			} else {
				if (!@mysql_select_db($this->request->post['db_name'], $connection)) {
					$this->error['warning'] = 'Fout: database bestaat niet';
				}
		...
		if (!$this->request->post['username']) {
			$this->error['username'] = 'Admin gebruikersnaam is verplicht';
		}

		if (!$this->request->post['password']) {
			$this->error['password'] = 'Admin wachtwoord is verplicht';
		}
		
		if (!is_numeric($this->request->post['rtlo'])) {
			$this->error['rtlo'] = 'TargetPay layoutcode is verplicht';
		}		

		if ((utf8_strlen($this->request->post['email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['email'])) {
			$this->error['email'] = 'Ongeldig e-mailadres';
		}
		
		if (!is_writable(DIR_OPENCART . 'config.php')) {
			$this->error['warning'] = 'Fout: config.php kon niet beschreven worden, controleer schrijfrechten voor: ' . DIR_OPENCART . 'config.php!';
		}
	
		if (!is_writable(DIR_OPENCART . 'admin/config.php')) {
			$this->error['warning'] = 'Fout: admin/config.php kon niet beschreven worden, controleer schrijfrechten voor: ' . DIR_OPENCART . 'admin/config.php!';
		}	
